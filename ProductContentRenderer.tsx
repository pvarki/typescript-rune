...
export class ProductContentRenderer {
     ...
     * @param compiler
     */
    public static register ( type: ContentType | string, compiler: RendererCompiler<BaseContent>) : void {
     ...
     * @param type
     */
    public static getRegistered ( type: ContentType | string ) : RendererCompiler<BaseContent> {
    ...
    public static compile (
        ...
        if (isString(content)) {
            const translate = this.isRegistered(ContentType.TRANSLATE) ? this.getRegistered(ContentType.TRANSLATE) : undefined;
            const translateFn = translate ? translate(createTranslateContent(content)) : undefined;
        ...
        return (context : RendererContext) => {
            ...
            const component : ComponentContent | undefined = context.contentService.getComponent(type);
            if (component) {
                const componentFn = this.compile(component);
                return <>{componentFn(context.createComponentContext(component, content))}</>
            }

            console.warn(`Warning! Unimplemented content type: ${content?.type}`);
            return <ErrorView title={`${content?.type}`} message={"Unimplemented content type"} />;
        };
