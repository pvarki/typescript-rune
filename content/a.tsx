...
export const a: RendererCompiler<BaseContent> = (
  ...
  const prepareHrefFn = prepareAHref(
    (content as unknown as { href?: string }).href,
  );
  return (context: RendererContext) => (
    <a className={prepareClassNameFn(context)} href={prepareHrefFn(context)}>
      {bodyFn(context)}
    </a>
  );
};
...
function prepareAHref(
  ...
  return (context: RendererContext) => {
    if (!value) return "";
    if (
      value.startsWith(ProductContentRenderer.ComponentParamPrefix) &&
