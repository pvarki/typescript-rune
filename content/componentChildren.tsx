...
export const componentChildren: RendererCompiler<BaseContent> = (
  content: BaseParentContent,
): CompiledRenderer => {
  // FIXME: Create customizable error component with title and message
  const errorFn = (title: string, message: string) => (
    <div>
      <h3>{title}</h3>
      <p>{message}</p>
    </div>
  ...
  return (context: RendererContext) => {
    ...
    if (isBaseParentContent(context.stateContent)) {
      return ProductContentRenderer.compile(context.stateContent.body)(
