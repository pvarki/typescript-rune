...
export function prepareClassName (
  ...
  return (context : RendererContext) => {
    ...
    return value.map( ( item : string ) : string => {
      if ( item.startsWith( ProductContentRenderer.ComponentParamPrefix ) && context?.componentContent && context?.stateContent ) {
        const key = item.substring( ProductContentRenderer.ComponentParamPrefix.length )
